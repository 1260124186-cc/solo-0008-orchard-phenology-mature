...
- 受控勘误：对已完成季节志提出受控更正，既可以修正日期/置信度/说明，也可以把误录阶段整体替换为正确阶段；采纳后形成勘误链，当前事实更新而原始结论、旧图谱和旧简报永久保留。
...
“错选阶段被替换”的完整例子（含环回 fruit_growth→petal_fall→fruit_growth，验证每跳去留、回到阶段单行、旧/新分析、重开恢复）：

```bash
PYTHONPATH=backend python3 scripts/verify_stage_replacement.py
```
...
node scripts/workflow_check.mjs --workflow replacement
```
...
- `replacement`：连续两次受控阶段替换（果实膨大期→落瓣期→果实膨大期，即环回），核对当前轨道只保留最初阶段一次、替换链两跳、最终阶段单行 restored 只显示来源、旧图谱/简报逐代冻结且只能显式接续新版。
...
- 完成后的日期或阶段录错只能通过受控勘误修正：`modify` 变更可修正已记录阶段的日期、置信度或说明；`replace` 变更把误录阶段整体替换为正确阶段，误录阶段从当前事实移除、正确阶段只出现一次。经采纳后按链顺序叠加为“当前生效事实”；季节志本体字节与修订号永不变更。
- 阶段替换不能移除完成季节志所必需的阶段（必需阶段只能替换为另一个必需阶段），也不能与当前已有阶段重复或违反阶段顺序。
- 连续替换按采纳顺序组成替换链（`replacement_chain`），每次移出/进入各成一跳；中间阶段标记为 `replaced_transit` 并同时记录来源与后续去向，当前轨道只保留最终阶段。若后续替换改回已出现过的阶段（环回 A→B→A），该阶段在谱系中仍只占一行并标记为 `restored`（只显示来源、不显示后续去向），每跳“是否仍在当前轨道”按最终事实集计算。
