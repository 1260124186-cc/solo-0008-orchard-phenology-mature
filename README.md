...
“错选阶段被替换”的完整例子（连续两跳 fruit_growth→petal_fall→bud_swell，验证替换前后、中间阶段谱系、旧/新分析、重开恢复）：
...
- `replacement`：连续两次受控阶段替换（果实膨大期→落瓣期→芽膨大期），核对当前轨道只保留最终阶段一次、替换链两跳完整、中间阶段标明来源与去向、旧图谱/简报逐代冻结且只能显式接续新版。
...
- 连续替换按采纳顺序组成替换链（`replacement_chain`），每次移出/进入各成一跳；中间阶段标记为 `replaced_transit` 并同时记录来源与后续去向，当前轨道只保留最终阶段。
